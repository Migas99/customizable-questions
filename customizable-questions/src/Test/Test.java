...
public class Test implements ITest {
    ...
    public boolean addQuestion(IQuestion iq) throws TestException {
        if (iq != null) {
            ...
            throw new TestException("A questão passada como parâmetro de entrada é null");
        }
        return false;
    }
                ...
                if (obj.has("type") && obj.has("question")) {
                    ...
                    if ("MultipleChoice".equals(type)) {
                        ...
                        if ("Numeric".equals(type)) {
                            this.addQuestion(new QuestionNumeric(
                                    question.get("title").getAsString(),
                                    question.get("question_description").getAsString(),
                                    question.get("mark").getAsFloat(),
                                    question.get("correct_answer").getAsDouble()
                            ));
                        } else {
                            if ("YesNo".equals(type)) {
                                this.addQuestion(new QuestionYesNo(
                                        question.get("title").getAsString(),
                                        question.get("question_description").getAsString(),
                                        question.get("mark").getAsFloat(),
                                        question.get("correct_answer").getAsString().toUpperCase()
                                ));
                            }
                        }
                    }
    ...
    public boolean saveTestResults() throws TestException {
        File dir = new File("results/test");
        int num;
        if (dir.exists() == true) {
        ...
        FileWriter arq = null;
        try {
            arq = new FileWriter("results/test" + "/Test" + String.valueOf(num) + ".txt", true);
        } catch (IOException ex) {
        ...
        for (IQuestion question : this.questions) {
            ...
            if (question instanceof QuestionMultipleChoice) {
                gravarArq.println("TipoPerg: QuestionMultipleChoice");
            } else {
                if (question instanceof QuestionNumeric) {
                    gravarArq.println("TipoPerg: QuestionNumeric");
                } else {
                    if (question instanceof QuestionYesNo) {
                        gravarArq.println("TipoPerg: QuestionYesNo");
                    }
                }
            }
